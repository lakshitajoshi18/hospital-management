...
import EditAppointmentDialogBox from "@/components/shared/EditAppointmentDialogBox";
...
const CheckAppointment = () => {
  ...
  const matchedAppointments = useMemo(
    () =>
      appointments.filter(
        (appointment) => appointment.mobile === searchedMobile.trim(),
      ),
    [searchedMobile],
  );
    ...
    <section className="relative overflow-hidden bg-[radial-gradient(circle_at_top_left,rgba(14,165,233,0.14),transparent_35%),linear-gradient(180deg,#f6fcff_0%,#ebf8fd_48%,#ffffff_100%)] px-4 py-14 sm:px-6 lg:px-8">
        ...
        <Card className="rounded-[30px] border border-white/80 bg-white/75 py-0 shadow-[0_24px_80px_rgba(14,116,144,0.12)] backdrop-blur">
          <CardHeader className="border-b border-cyan-100/80 pb-5 pt-6">
            ...
            <CardTitle className="pt-2 text-2xl text-slate-950 sm:text-3xl">
              Check Your Registered Appointments
            </CardTitle>
            <CardDescription className="text-base text-slate-600">
              Enter your mobile number and view appointment history instantly in
              a detailed dialog.
            </CardDescription>
          </CardHeader>
          ...
          <CardContent className="p-5 sm:p-6">
            ...
            <p className="mt-3 text-sm text-slate-600">
              ...
              <span className="font-semibold text-cyan-800">7896541230</span>
            </p>
          </CardContent>
        </Card>
      </div>
        ...
        <DialogContent className="max-h-[88vh] overflow-hidden border-cyan-100 bg-[linear-gradient(180deg,#f9fdff_0%,#f2faff_100%)] p-0 sm:max-w-4xl">
          <DialogHeader className="border-b border-cyan-100 px-5 pb-4 pt-5 sm:px-6">
            <DialogTitle className="text-xl text-slate-900 sm:text-2xl">
              Appointments for {searchedMobile}
            </DialogTitle>
                    ...
                    {list.map((appointment) => (
                      ...
                      >
                        ...
                        <TableCell className="text-slate-700">
                          {appointment.hospital ?? "Not Appointed yet"}
                        </TableCell>
                        <TableCell
                          className={`${appointment.doctor ? "text-slate-900" : "text-red-700"}`}
                        >
                        ...
                        <TableCell className="max-w-55 truncate text-slate-700">
                          {appointment.problem}
                        </TableCell>
                        <TableCell>
      ...
      <EditAppointmentDialogBox
        ...
        onSaved={(updatedAppointment) => {
          ...
          if (searchedMobile) {
            getAppointmentList({ mobile: searchedMobile });
          }
        }}
      />
    </section>
