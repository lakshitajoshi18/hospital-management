...
const authMiddleware = async () => {
  ...
  try {
    const response = await clientFetch(
      `/api/doctors/me?uid=${encodeURIComponent(doctorId)}`,
    );
...
export const useDoctorStore = create<DOCTORSTOREINTERFACE>((set, get) => ({
  ...
  getAppointmentList: async () => {
    try {
      ...
      if (!currentUser?.id || !currentUser?.hospital?.id) {
        console.warn(
          "getAppointmentList skipped because user or hospital data is missing",
          currentUser,
        );
        set({ appointmentList: [] });
        return;
  ...
  getAppointedPatientList: async () => {
    try {
      ...
      if (!currentUser?.id || !currentUser?.hospital?.id) {
        console.warn(
          "getAppointedPatientList skipped because user or hospital data is missing",
          currentUser,
        );
  ...
  getWeeklyAppointments: async () => {
    try {
      ...
      const emptyWeekData: WeeklyAppointments[] = dayLabels.map((day) => ({
        day,
        appointments: 0,
      }));
      const currentUser = get().user;
      ...
      response.forEach((item) => {
        ...
        const appointmentDate = new Date(
          item.appointmentDate as unknown as string,
        );
      ...
      const weeklyAppointmentsData: WeeklyAppointments[] = dayLabels.map(
        (day, index) => ({
          day,
